...
{
    internal sealed class RuntimeSegmentZoomCoordinator
    {
        ...
        public RuntimeSegmentZoomCoordinator(
        ...
        public void ApplyText()
        {
            ...
            controls.ResetButton.Text = UiText.CurrentLanguage == UiLanguage.English ? "Full" : "전체";
        }

        public void Update()
        {
            var state = getState();
            controls.RangeLabel.Text = UiText.CurrentLanguage == UiLanguage.English
                ? $"Runtime view: {state.ViewRangeText}"
                : $"실행 구간 보기: {state.ViewRangeText}";
