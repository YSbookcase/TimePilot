    ...
    internal static class SystemTimelineEventTextFormatter
    {
        ...
        public static string GetRelationText(string eventType)
        {
            ...
            return eventType.ToLowerInvariant() switch
            {
                ...
                "timepilot-start" => isEnglish ? "ActiveLogbook recording start" : "ActiveLogbook 기록 시작",
                "timepilot-exit" => isEnglish ? "ActiveLogbook recording end" : "ActiveLogbook 기록 종료",
        ...
        public static string FormatDetails(SystemTimelineEvent systemEvent)
        {
            ...
            {
                return UiText.CurrentLanguage == UiLanguage.English
                    ? $"ActiveLogbook started at {startedAt.ToLocalTime():HH:mm:ss}"
                    : $"ActiveLogbook 시작 {startedAt.ToLocalTime():HH:mm:ss}";
            }
        ...
        public static string GetInferredDetailsText(string eventType)
        {
            ...
            return eventType.ToLowerInvariant() switch
            {
                ...
                "recording-end-estimate" => isEnglish
                    ? "Estimated from the last ActiveLogbook runtime record."
                    : "ActiveLogbook의 마지막 실행 기록을 기준으로 추정했습니다.",
                _ => "-"
            };
        }
    }
