            ...
            public static UiTextResources CreateKorean()
            {
                return new UiTextResources(
                    ...
                    new MainText(
                        ...
                        DataRestoreWarning: "전체 복원은 현재 사용 기록 데이터베이스와 설정을 백업 파일의 내용으로 대체합니다.\n\n백업 이후 현재 PC에서 쌓인 기록은 현재 데이터에서 사라질 수 있으며, 해당 기간은 타임라인에서 `TimePilot 미실행`으로 보일 수 있습니다.\n\n복원 전에 현재 데이터는 안전 백업으로 자동 저장됩니다. 복원 후 TimePilot은 현재 세션을 다시 시작합니다. 계속할까요?",
            ...
            public static UiTextResources CreateEnglish()
            {
                return new UiTextResources(
                    ...
                    new MainText(
                        ...
                        DataRestoreWarning: "Full restore replaces the current usage database and settings with the backup contents.\n\nRecords collected on this PC after the backup may disappear from the current data, and that period can appear as `TimePilot not running` in the timeline.\n\nTimePilot will automatically create a safety backup of the current data before restoring. TimePilot will restart the current recording session after restore. Continue?",
