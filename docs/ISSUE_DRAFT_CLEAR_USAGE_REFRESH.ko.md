...
- [ ] 다른 PC에서 실제 `timepilot.db`와 `settings.json`의 경로를 확인한다. 패키지별 AppData 리디렉션 여부도 확인한다.
...
- 실제 데이터 파일 위치를 환경설정의 폴더 열기 기능으로 확인할 수 있다.
...
이 문서는 GitHub Issue #320의 로컬 기록이다. 다른 PC에서 재실행 후 보인 행이 삭제 전 기록인지 새 기록인지 확인되면 결과를 Issue에 추가한다.
