...
이 문서는 ActiveLogbook을 Microsoft Store에 제출하기 전과 후의 비개발 준비 항목을 추적한다. 2026-09-12 제출의 실제 상태와 검증 기록은 `docs/release/STORE_SUBMISSION_RECORD_0.2.3.ko.md`를 참고한다.
...
- [x] Microsoft Store 앱 이름을 예약하기 전에 공개 제품명을 확정한다.
- [x] Store 브랜드를 확정하기 전에 `docs/release/BRAND_NAMING_REVIEW.ko.md`를 검토한다.

## Partner Center

- [x] Microsoft Partner Center 개발자 계정을 만들거나 확인한다.
- [x] 앱 이름을 예약한다.
- [x] 제출 전에 게시자 표시 이름을 확인한다.
- [x] 카테고리와 연령 등급을 선택한다.
- [x] 지원 연락처를 준비한다.
...
- [x] 짧은 앱 설명
- [x] 자세한 앱 설명
- [x] 로컬 사용 시간 추적 중심의 기능 목록
- [x] 요약, 타임라인, 상세, 설정, 트레이 동작 스크린샷
- [x] 앱 아이콘과 Store 이미지
- [x] 릴리스 노트
- [x] 알려진 제한사항
- [ ] GitHub 저장소 링크
...
- [x] Partner Center에 개인정보처리방침 URL을 입력한다.
- [x] README 또는 앱 지원 문서에서 개인정보처리방침을 연결한다.
- [x] 사용 기록 데이터가 기본적으로 로컬에 저장된다는 점을 설명한다.
- [x] ActiveLogbook은 기본적으로 사용 기록을 개발자 서버로 전송하지 않는다는 점을 설명한다.
