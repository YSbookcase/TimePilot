...
- `TimePilot.WinForms/KYS24/Features`: Community/Pro 후보 기능, 메뉴, 탭, 설정 섹션, 분석 패널, 내보내기 작업을 등록하기 위한 내부 레지스트리.
- `TimePilot.WinForms/KYS24/Analytics`: 기존 저장소를 읽기 전용으로 감싸는 일별 분석 계약. 연간 달력 히트맵 같은 장기 통계 기능이 기존 SQLite 저장소에 직접 결합하지 않도록 하는 준비 단계다.

현재 `Analytics` 계약은 날짜별 활성 사용 시간, 유휴 기록 시간, 대표 앱, 기록 상태 지표를 제공하는 범위에 한정한다. Pro UI, 라이선스 확인, 동적 모듈 로딩은 포함하지 않는다.

## 5. 아직 완료되지 않은 구조 작업

- `TimePilot.Core`, `TimePilot.Infrastructure`, `TimePilot.WinForms`의 실제 프로젝트 분리
- WinForms와 저장소 사이의 더 넓은 읽기 전용 분석 계약
- 메뉴, 분석 화면, 설정 섹션을 실제 화면 흐름에 연결하는 확장 계약
...
- 날짜별 주요 사용 분류를 기본 신호로 표시
- 활성 사용 시간은 색상 진하기나 보조 지표로 표현
- 날짜 hover 또는 선택 패널에서 활성 시간, 유휴 기록 시간, 대표 앱, 기록 상태 표시
